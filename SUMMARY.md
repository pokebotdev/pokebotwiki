# Table of contents

* [Welcome to the Pokebot docs!](README.md)
* [FAQ](welcome.md)
* [Special Feature](special-feature.md)

## Commands

* [Pokedex \(pd\) Command](commands/pd/README.md)
  * [Pokedex Alola \(pd alola\) Command](commands/pd/pokedex-alola-pd-alola-command.md)
  * [Pokedex Mega \(pd mega\) Command](commands/pd/pokedex-mega-pd-mega-command.md)
* [Ability \(abil\) Command](commands/abil.md)
* [Category \(cat\) Command](commands/cat.md)
* [Color \(color/colour\) Command](commands/color.md)
* [Type \(type\) Command](commands/type.md)
* [Information \(info\) Command](commands/info.md)
* [Feedback \(fb\) Command](commands/fb.md)
* [Setprefix \(prefix\) Command](commands/prefix.md)
* [Stats \(stat\) Command](commands/stat.md)
* [Votes \(vote\) Command](commands/vote.md)
* [DMs \(dm\) Command](commands/dm.md)
* [Pokecord Prefix \(pc\) Command](commands/pc.md)

